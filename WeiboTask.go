...
func updateCookieLoop() {
	...
	for {
		go updateCookie(true)
		time.Sleep(16 * time.Hour)
	}
}
func updateCookie(reloadConfig bool) {
	if reloadConfig {
		err := LoadConfig()
		if err != nil {
			log.Println("配置文件加载失败:"+err.Error())
	...
	if wb.LoginByCookies(MyConfig.Cookies) {
		...
		sub  := wb.GetCookie("SUB",".m.weibo.com");
		//更新db
		var sina_config model.SinaConfig
		sina_config.SinaConfigSave(model.SinaConfig{
				Name :"SUB",
				Value: sub,
			})
